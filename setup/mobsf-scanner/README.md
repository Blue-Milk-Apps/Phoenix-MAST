...
Phoenix can use MobSF for IPA and APK binary analysis. The scanner talks to a running MobSF API service instead of a local binary command, and it only runs when `MOBSF_URL` is configured.

## Start MobSF Only

Start the MobSF sidecar without running the full Phoenix Docker workflow:
...
- API errors: make sure `MOBSF_API_KEY` matches on both the MobSF service and the Phoenix scan command.
