"""Normalized access to React Native source scan outputs."""
...
class ReactNativeScanExtractionContext:
    """Provide defensive typed views over persisted React Native scan artifacts."""
    ...
    def framework(self) -> dict[str, Any]:
        return self._mapping(self.source_metadata.get("framework"))

    @property
    def engines(self) -> dict[str, Any]:
        return self._mapping(self.source_metadata.get("engines"))

    @property
    def entrypoints(self) -> dict[str, Any]:
        return self._mapping(self.source_metadata.get("entrypoints"))
    ...
    def dependencies(self) -> dict[str, list[dict[str, Any]]]:
        ...
        return {key: self._mapping_list(values.get(key)) for key in ("direct", "development")}
    ...
    def opengrep_configured_rule_ids(self, scope: str) -> frozenset[str]:
        return frozenset(self.string_list(self.opengrep_scope(scope).get("configured_rule_ids")))
    ...
    def _opengrep_result_scope(result: dict[str, Any]) -> str:
        ...
        if rule_id.startswith(("react-native.", "react_native.", "javascript.", "typescript.")):
            return "react_native"
