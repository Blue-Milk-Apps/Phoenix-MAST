...
- `pdf_report/` - the PDF output adapter. `PdfReportGenerator` renders
  standard `ReportData` with `templates/report.html.jinja`, draws the polar
  risk chart with matplotlib, and converts the result to PDF with WeasyPrint.
...
Reports are generated by the Phoenix scan workflow after post-scan processing.
The workflow builds standard `ReportData` with `ReportGenerationService` and
passes it to `PdfReportGenerator`. External callers that already have a
`post_scan_processing.json` artifact can use the same services:
...
post_scan_data = json.loads(Path("post_scan_processing.json").read_text())
...
]).build_report_data(post_scan_data)
...
2. Fill in each section:
   ...
   - `app_info` - name, package, SDK versions, plus the app-summary-card
     ...
     `trackers_detected`.
   - `functionality` - each detected capability with `present: true/false`,
     an explanation, and optional per-platform assessments. Missing evidence is
     rendered as `Not Evaluated` rather than as a clean result.
   ...
   - `vulnerability_sections` - one object per category. Code, Network, Data
     Storage, and Resilience sections each have a `findings_text` narrative
     paragraph and a `checks` array. Each check may be `Present`, `Not Present`,
     `Partial`, `Not Evaluated`, or `Not Applicable` and includes its
     explanation, compliance, remediation link, and evidence.
