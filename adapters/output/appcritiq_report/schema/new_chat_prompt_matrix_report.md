# Starter prompt -- AppCritique check schema + matrix report review
...
I'm building a security-report tool called **AppCritique** -- a Python +
...
## Check schema built from capabilities.csv + AppCritique's own fields

I built a JSON Schema for a single security check record
(`check.schema.json`), deliberately scoped to only two sources:
capabilities.csv's own columns, and the field set AppCritique's own
...
- `impact` = plain-language consequence of THIS finding specifically
  (AppCritique's Explanation column) -- distinct from `description`.
- `status` = Present / Not Present / Not Evaluated (AppCritique's Result
...
- `compliance` folds in HIPAA/GDPR/legacy-OWASP-Mobile-Top-10 tags too,
  since those appear in AppCritique's own Compliance column even though
...
## Full capabilities.csv -> AppCritique mapping (Android, everything I'd already worked out)
...
fully covered by a named AppCritique check or report field; 2 were out of
...
and don't limit new fields to only what would render in the AppCritique
