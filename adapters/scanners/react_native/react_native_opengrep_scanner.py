...
class ReactNativeOpenGrepScanner(ScannerPort):
    """Run React Native and embedded native rules against their intended trees."""

    REPORT_PATH = "opengrep_results.json"
    SOURCE_SUFFIXES = frozenset({".js", ".jsx", ".mjs", ".cjs", ".ts", ".tsx"})
    ...
    def description(self) -> str:
        return "Scoped React Native, Android, and iOS source-code security analysis with OpenGrep."
    ...
    def scan(self, config: ScanConfig) -> list[ScanResult]:
        ...
        scope_specs = {
            "react_native": {
                ...
                "scan_paths": self._react_native_scan_paths(project_path),
            },
            "android": {
                "required": False,
        ...
        *,
        ...
        if not scan_paths:
            base_metadata["reason"] = (
                "No JavaScript or TypeScript application source paths were found."
        ...
        if rules_path is None or not rules_path.is_dir():
            base_metadata["reason"] = f"No {scope.replace('_', ' ')} OpenGrep rules directory was found."
            return base_metadata, [], [], ""
    ...
    @classmethod
    def _react_native_scan_paths(cls, project_path: Path) -> list[Path]:
        paths = [
            path
            for path in sorted(project_path.iterdir())
            if path.is_dir()
            and path.name not in cls.EXCLUDED_DIRECTORIES
            and any(candidate.suffix.lower() in cls.SOURCE_SUFFIXES for candidate in path.rglob("*"))
        ]
        paths.extend(
            path
            for path in sorted(project_path.iterdir())
            if path.is_file() and path.suffix.lower() in cls.SOURCE_SUFFIXES
        )
        return paths
