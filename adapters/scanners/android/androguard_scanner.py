...
class AndroguardScanner(ScannerPort):
    ...
    STRING_PATTERNS = {
        ...
        "api_key": re.compile(r"(?i)(api[_-]?key|apikey|client[_-]?secret|secret[_-]?key)"),
        "token": re.compile(r"(?i)(access[_-]?token|auth[_-]?token|bearer|oauth|\btoken=)"),
        ...
        "auth": re.compile(r"(?i)(password|passwd|credential|login|signin|auth|session)"),
        "environment_marker": re.compile(r"(?i)(debug|staging|stage|dev|qa|prod|production|sandbox)"),
        "filesystem_path": re.compile(r"(?i)(/data/|/sdcard/|/storage/|/system/|content://|file://)"),
        "sql": re.compile(
            r"(?i)(\bselect\b.+\bfrom\b|\binsert\s+into\b|"
            r"\bupdate\b.+\bset\b|\bdelete\s+from\b|\bpragma\b|\bsqlite\b)"
        ),
        "reflection": re.compile(r"(?i)(forName|getMethod|getDeclaredMethod|invoke|reflect)"),
        "dynamic_loading": re.compile(r"(?i)(DexClassLoader|PathClassLoader|loadLibrary|loadClass|System\.load)"),
    }
    SENSITIVE_API_PATTERNS = {
        ...
        "tls": re.compile(r"Ljavax/net/ssl/|HostnameVerifier|TrustManager|X509TrustManager"),
        ...
        "runtime_exec": re.compile(r"Ljava/lang/Runtime;->exec|Ljava/lang/ProcessBuilder;"),
        ...
        "permissions": re.compile(r"checkSelfPermission|requestPermissions|checkCallingPermission"),
        "location": re.compile(r"Landroid/location/|FusedLocationProviderClient"),
        "keystore": re.compile(r"KeyStore|KeyGenParameterSpec|Landroid/security/keystore/"),
    }
    ...
    def _extract_metadata(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
    ...
    def _extract_manifest(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
    ...
    def _extract_permissions(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        apk = androguard_context.apk
        return {
            "requested": sorted(self._call(apk, "get_permissions", []) or []),
            "declared": sorted((self._call(apk, "get_declared_permissions", {}) or {}).keys()),
        }

    def _extract_components(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        apk = androguard_context.apk
        ...
        return {
            ...
            "services": self._component_records(apk, manifest, "service", self._call(apk, "get_services", []) or []),
            "receivers": self._component_records(apk, manifest, "receiver", self._call(apk, "get_receivers", []) or []),
            "providers": self._component_records(apk, manifest, "provider", self._call(apk, "get_providers", []) or []),
        }
    ...
    def _extract_strings(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        ...
        for string_analysis in analysis.get_strings():
            ...
            xrefs = [
                ...
                for class_analysis, method_analysis in list(string_analysis.get_xref_from())[: self.MAX_STRING_XREFS]
            ]
            items.append(
                {
                    "value": value,
    ...
    def _extract_xrefs(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        items: list[dict[str, Any]] = []
    ...
    def _extract_native_libs(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        return {
            "items": [
                name
    ...
    def _extract_assets(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        return {"items": [name for name in self._apk_names(androguard_context.apk_path) if name.startswith("assets/")]}
    ...
    def _extract_files(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        return {"items": self._apk_names(androguard_context.apk_path)}

    def _extract_findings(self, androguard_context: AndroguardScanContext) -> dict[str, Any]:
        findings: list[dict[str, Any]] = []
        permissions = set(self._call(androguard_context.apk, "get_permissions", []) or [])
    ...
    def _build_report_summary(
        ...
        return {
            ...
            "finding_severity_counts": self._count_values(finding.get("severity", "") for finding in findings),
            ...
            "api_category_counts": self._count_values(
                category for item in api_calls for category in item.get("categories", [])
            ),
    ...
    def _finding_string_evidence(self, androguard_context: AndroguardScanContext) -> dict[str, list[dict[str, Any]]]:
    ...
    def _finding_api_evidence(self, androguard_context: AndroguardScanContext) -> dict[str, list[dict[str, Any]]]:
        categories_of_interest = {
            "runtime_exec",
            "dynamic_loading",
            "tls",
    ...
    def _component_attack_surface(self, androguard_context: AndroguardScanContext) -> list[dict[str, Any]]:
        apk = androguard_context.apk
        manifest = self._call(apk, "get_android_manifest_xml")
        evidence: list[dict[str, Any]] = []
        component_getters = {
                ...
                evidence.append(
                    {
                        ...
                        "name": name,
                        "exported": record["exported"],
                        "implicitly_exported": record["exported"] is None and record["has_intent_filters"],
    ...
    def _string_categories(self, value: str) -> list[str]:
        if len(value.strip()) < 4:
            return []
        return [category for category, pattern in self.STRING_PATTERNS.items() if pattern.search(value)]
    ...
    def _api_categories(self, method_analysis: Any) -> list[str]:
        ...
        return [category for category, pattern in self.SENSITIVE_API_PATTERNS.items() if pattern.search(signature)]
    ...
    def _component_records(
        ...
        return [self._component_record(apk, manifest, component_type, name) for name in sorted(component_names)]
    ...
    def _component_record(
        ...
        return {
            ...
            "exported": self._boolean_manifest_value(
                self._manifest_component_attribute(apk, manifest, component_type, name, "exported")
            ),
            "permission": self._manifest_component_attribute(apk, manifest, component_type, name, "permission"),
