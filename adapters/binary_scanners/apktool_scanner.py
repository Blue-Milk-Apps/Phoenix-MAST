"""Apktool scanner adapter for compact Android security evidence extraction."""
...
class ApktoolScanner(ScannerPort):
    ...
    CODE_PATTERNS = {
        ...
        "dynamic_loading": re.compile(
            r"(DexClassLoader|PathClassLoader|loadClass|loadLibrary|System;->load)"
        ),
        ...
        "trust_manager": re.compile(
            r"(Ljavax/net/ssl/|X509TrustManager|HostnameVerifier|TrustManager)"
        ),
    ...
    ENDPOINT_PATTERNS = {
        ...
        "secret_keyword": re.compile(
            r"(?i)(api[_-]?key|client[_-]?secret|secret[_-]?key|access[_-]?token)"
        ),
    }
    ...
    def scan(self, config: ScanConfig) -> list[ScanResult]:
        ...
        try:
            ...
            artifacts["decode_metadata.json"] = self._decode_metadata(
                apk_path, decode_result, artifacts, errors
            )
    ...
    def _extract_manifest_summary(self, decoded_root: Path) -> dict[str, Any]:
        ...
        return {
            ...
            "version_code": self._android_attr(root, "versionCode")
            or apktool_metadata.get("version_code", ""),
            "version_name": self._android_attr(root, "versionName")
            or apktool_metadata.get("version_name", ""),
            "application": {
                "debuggable": self._android_attr(application, "debuggable"),
                "allow_backup": self._android_attr(application, "allowBackup"),
                "uses_cleartext_traffic": self._android_attr(
                    application, "usesCleartextTraffic"
                ),
                "network_security_config": self._android_attr(
                    application, "networkSecurityConfig"
                ),
            },
            "provenance": self._provenance(
                decoded_root, decoded_root / "AndroidManifest.xml"
            ),
        }
    ...
    def _extract_deep_links(self, decoded_root: Path) -> dict[str, Any]:
        ...
        for activity in manifest.findall(".//activity"):
            ...
            for intent_filter in activity.findall("intent-filter"):
                actions = [
                    self._android_attr(action, "name")
                    for action in intent_filter.findall("action")
                ]
                categories = [
                    self._android_attr(category, "name")
                ...
                for data in intent_filter.findall("data"):
                    ...
                    value = "://".join(
                        part for part in (context["scheme"], context["host"]) if part
                    )
                    links.append(
                        self._evidence(
                            "deep_link",
        ...
        return {
            "deep_links": sorted(
                links, key=lambda item: json.dumps(item, sort_keys=True)
            )
        }
    ...
    def _extract_network_security_config(self, decoded_root: Path) -> dict[str, Any]:
        ...
        if config_path is None or not config_path.exists():
            return {
                ...
                "manifest_uses_cleartext_traffic": manifest_summary["application"][
                    "uses_cleartext_traffic"
                ],
                "target_sdk": manifest_summary["target_sdk"],
                "effective_cleartext_traffic_default": self._cleartext_default(
                    manifest_summary
                ),
        ...
        for domain_config in root.findall(".//domain-config"):
            domains.append(
                {
                    "cleartext_permitted": domain_config.attrib.get(
                        "cleartextTrafficPermitted", ""
                    ),
                    "domains": [
                        {
                            "value": (domain.text or "").strip(),
                            "include_subdomains": domain.attrib.get(
                                "includeSubdomains", ""
                            ),
                        }
    ...
    def _extract_code_indicators(self, decoded_root: Path) -> dict[str, Any]:
        return {
            "items": self._scan_text_patterns(
                decoded_root, self.CODE_PATTERNS, "code_indicator"
            )
        }
    ...
    def _resource_reference_path(
        self, decoded_root: Path, reference: str
    ) -> Path | None:
    ...
    def _decode_metadata(
        ...
        decoded_files_used = sorted(
            self._artifact_sources(artifact)
            for artifact in artifacts.values()
            if isinstance(artifact, dict)
        )
        decoded_files_used = sorted(
            {source for sources in decoded_files_used for source in sources}
        )
    ...
    def _scan_results(
        ...
        extracted_any = any(
            name not in {"decode_metadata.json", "extraction_errors.json"}
            and self._count_items(value) > 0
        ...
        return [
            ScanResult(
                ...
                error_message=""
                if overall_success
                else self._first_error(decode_result, errors),
