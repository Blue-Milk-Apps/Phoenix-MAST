...
from adapters.binary_scanners import apkid_scanner
...
def test_apkid_extracts_normalized_operational_intelligence(
    ...
    def fake_run(cmd, capture_output, text, check, timeout):
        ...
        targets = cmd[2:]
        dex_target = next(
            target for target in targets if target.endswith("classes.dex")
        )
        stdout = json.dumps(
            {
                "rules_sha256": "abc123",
    ...
    assert (
        evidence["extraction_metadata"]["rule_signature_metadata"]["rules_sha256"]
        == "abc123"
    )
    ...
    assert (
        ...
        in by_family["packer"]["analysis_impacts"]
    )
    ...
    assert any(
        hint["tools"]
        == ["JADX", "apktool", "Androguard", "Frida", "runtime_instrumentation"]
