...
from adapters.binary_scanners import apktool_scanner
...
def test_apktool_partial_decode_extracts_normalized_evidence(
    ...
    assert (
        "AndroidManifest.xml" in artifacts["decode_metadata.json"]["decoded_files_used"]
    )
    ...
    assert (
        artifacts["attack_surface.json"]["components"][0]["context"]["exported"]
        == "true"
    )
    assert (
        artifacts["deep_links.json"]["deep_links"][0]["context"]["host"]
        == "example.com"
    )
    assert (
        artifacts["network_security_config.json"]["domains"][0]["domains"][0]["value"]
        == "api.example.com"
    )
    assert artifacts["network_security_config.json"]["target_sdk"] == "22"
    assert artifacts["code_indicators.json"]["items"][0]["provenance"]["path"].endswith(
        ".smali"
    )
    assert artifacts["secrets_endpoints.json"]["items"]
    assert artifacts["native_libraries.json"]["libraries"][0]["abi"] == "arm64-v8a"
    assert (
        artifacts["assets_inventory.json"]["assets"][0]["path"] == "assets/config.json"
    )
