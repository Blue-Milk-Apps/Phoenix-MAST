...
from adapters.binary_scanners import aapt2_scanner
...
def test_aapt2_extracts_normalized_evidence(monkeypatch, tmp_path: Path) -> None:
    ...
    assert (
        evidence["extraction_metadata"]["aapt2_version"]
        == "Android Asset Packaging Tool (aapt) 2.0"
    )
    ...
    assert any(
        ...
        and permission["protection_level_hint"] == "dangerous"
        for permission in evidence["permissions"]
    )
    assert any(
        component["component_type"] == "provider"
        and component["name"] == "androidx.core.content.FileProvider"
        for component in evidence["components"]
    )
    assert evidence["intent_filters"][0]["uri_patterns"][0]["normalized"] == (
        "https://example.com/oauth/callback"
    )
    assert evidence["intent_filters"][0]["auth_related_entrypoint_indicator"] is True
    assert any(
        candidate["name"] == "network_security_config"
    ...
    assert all(
        candidate["not_a_finding"]
        for candidate in evidence["candidate_interpretations"]
    )
    ...
    assert any(
        item["name"] == "components.json" and item["item_count"] == 2
        for item in scan_index["artifacts"]
    )
...
def test_aapt2_tolerates_partial_command_failure(monkeypatch, tmp_path: Path) -> None:
    ...
    scan_index = output_json(results, "scan_index.json")
    assert any(
        item["name"] == "resource_candidates.json" and item["partial_failure"]
        for item in scan_index["artifacts"]
    )
